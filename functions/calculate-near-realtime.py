...
def handler(event, context):
    ...
    try:
        ...
        kinesisCost = compute_kinesis_cost(resource_manager, tagkey, tagvalue, pricing_records, start, end)
        ...
        log.info("Estimated monthly cost for resources tagged with key={},value={} : [{}]".format(tagkey, tagvalue,
                                                                                                  json.dumps(result,
                                                                                                             sort_keys=False,
                                                                                                             indent=4)))
...
def compute_kinesis_cost(resource_manager, tagkey, tagvalue, pricing_records, start, end):
    kinesisCost = 0
    try:
        # Kinesis Streams
...
def compute_dynamo_cost(resource_manager, tagkey, tagvalue, pricing_records):
    ...
    try:
        ...
        if totalRead and totalWrite:
            ddbpdim = data.DynamoDBPriceDimension(region=region,
                                                  readCapacityUnitHours=totalRead * HOURS_DICT[FORECAST_PERIOD_MONTHLY],
                                                  writeCapacityUnitHours=totalWrite * HOURS_DICT[
...
def compute_lambda_cost(resource_manager, tagkey, tagvalue, pricing_records, start, end):
    ...
    try:
        ...
        for func in lambdafunctions:
            ...
            log.info("Executions for Lambda function [{}]: [{}] - Memory:[{}] - Avg Duration:[{}]".format(funcname,
                                                                                                          executions,
                                                                                                          memory,
                                                                                                          avgduration))
            if executions and avgduration:
                try:
                    ...
                    lambdapdim = data.LambdaPriceDimension(region=region,
                                                           requestCount=executions * calculate_forecast_factor(),
                                                           avgDurationMs=avgduration, memoryMb=memory,
                                                           dataTranferOutInternetGb=0,
                                                           dataTranferOutIntraRegionGb=0, dataTranferOutInterRegionGb=0,
                                                           toRegion='')
                ...
                log.info(
                    "Skipping pricing calculation for function [{}] - qualifier [{}] due to lack of executions in [{}-minute] time window".format(
                        fullname, qualifier, METRIC_WINDOW))
...
def compute_ec2_cost(resource_manager, tagkey, tagvalue, pricing_records, start, end):
    ...
    try:
        ...
        if elb_instances:
            try:
                log.info(
                    "Found registered EC2 instances to tagged ELBs [{}]:{}".format(taggedelbs, elb_instances.keys()))
                elb_data_processed_gb = calculate_elb_data_processed(start, end,
                                                                     elb_instances) * calculate_forecast_factor() / (10 ** 9)
            except Exception as failure:
        ...
        if elb_hours:
            elb_cost = ec2pricing.calculate(
                data.Ec2PriceDimension(region=region, elbHours=elb_hours, elbDataProcessedGb=elb_data_processed_gb))
            if 'pricingRecords' in elb_cost:
                pricing_records.extend(elb_cost['pricingRecords'])
                ec2Cost = ec2Cost + elb_cost['totalCost']

        # Calculate Application Load Balance cost
        if alb_hours:
        ...
        for r in config_regions:
            ec2_client = boto3.client('ec2', r)
            ec2_instances = get_ec2_instances_by_tag(ec2_client, tagkey, tagvalue)
            total_ebs_cost = total_ebs_cost + compute_ebs_cost(ec2_client, pricing_records, ec2_instances)
        ...
        for instance_type in all_instance_types:
            try:
                ec2_compute_cost = ec2pricing.calculate(
                    data.Ec2PriceDimension(region=region, instanceType=instance_type,
                                           instanceHours=all_instance_types[instance_type] * HOURS_DICT[DEFAULT_FORECAST_PERIOD]))
        ...
        traceback.print_exc()
        log.error("Exception message:[" + str(e) + "]")
    log.info("total ec2 cost %s", ec2Cost)
    log.info("total ebs cost %s", total_ebs_cost)
    return ec2Cost + total_ebs_cost

def compute_ebs_cost(ec2_client, pricing_records, ec2_instances):
    ebs_cost= 0
    ebs_storage_dict, piops = get_storage_by_ebs_type(ec2_client, ec2_instances)
    ...
    try:
        ...
        for db_instance_type in all_db_instance_types:
            try:
                ...
                rds_instance_cost = rdspricing.calculate(
                    data.RdsPriceDimension(region=region, dbInstanceClass=dbInstanceClass, multiAz=multiAz,
                                           engine=engine, licenseModel=licenseModel,
                                           instanceHours=all_db_instance_types[db_instance_type] * HOURS_DICT[
...
def get_ec2_instances_by_tag(ec2_client, tagkey, tagvalue):
    result = {}
...
def calculate_elb_data_processed(start, end, elb_instances):
    ...
    log.info(
        "Total Bytes processed by ELBs in time window of [" + str(METRIC_WINDOW) + "] minutes :[" + str(result) + "]")
...
def calculate_alb_lcus(start, end, albs):
    ...
    log.info("Total ConsumedLCUs consumed by ALBs in time window of [" + str(METRIC_WINDOW) + "] minutes :[" + str(
        result) + "]")
...
def get_lambda_memory(functionname, qualifier):
    ...
    args = {}
    if qualifier:
        args = {'FunctionName': functionname, 'Qualifier': qualifier}
    else:
...
def calculate_kinesis_put_payload_units(start, end, streamName):
    ...
    try:
        ...
        log.info("get_kinesis_stream_puts - incomingRecords:[{}] - chunkAvg:[{}] - totalPutPayloadUnits:[{}]".format(
            totalRecords, chunkCount / bytesDatapoints, totalPutPayloadUnits))
...
class ResourceManager():
    ...
    def init_resources_all_region(self, tagkey, tagvalue):
        ...
        for r in config_regions:
            log.info('init resources in %s', r)
    ...
    def extract_resource(self, arn):
        ...
        for service in (SERVICE_EC2, SERVICE_ELB, SERVICE_DYNAMODB, SERVICE_KINESIS):
            ...
            for type in (
            RESOURCE_EC2_INSTANCE, RESOURCE_EBS_VOLUME, RESOURCE_EBS_SNAPSHOT, RESOURCE_DDB_TABLE, RESOURCE_STREAM):
                if ':' + service + ':' in arn and ':' + type + '/' in arn:
                    resourceId = arn.split(':' + type + '/')[1]
                    return self.Resource(service, type, resourceId, arn)
